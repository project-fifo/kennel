...
upgrade(Req, _Env, _Handler, State = #{uuid := _UUID}, infinity, run) ->
    %% TODO: add stream handler: https://github.com/ninenines/cowboy/issues/1171
    {error, meh, Req, State}.
    ...
    %%         {ok, Console} = libchunter:console_open(
    %% Host, Port, UUID, self()),
